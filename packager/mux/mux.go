...
func New(certCache http.Handler,
         signer http.Handler,
				 validityMap http.Handler,
				 healthCheck http.Handler) http.Handler {
	return &mux{certCache, signer, validityMap, healthCheck}
}
...
func (this *mux) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	} else if path == util.HealthCheckPath {
		this.healthCheck.ServeHTTP(resp, req)
	} else {
		http.NotFound(resp, req)
	}
}
